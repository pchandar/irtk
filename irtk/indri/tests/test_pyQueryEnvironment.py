...
class TestPyQueryEnvironment:
    ...
    @classmethod
    def setup_class(cls):
        cls.env.add_index(path.dirname(__file__) + '/data/tinyTestCollection/index')

        with open(path.dirname(__file__) + '/data/tinyTestCollection/docIDs.txt') as f:
            cls.true_docids = [docid.strip() for docid in f]

        with open(path.dirname(__file__) + '/data/tinyTestCollection/cfanddf.txt') as f:

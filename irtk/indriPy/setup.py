...
def configuration(parent_package='', top_path=None):
    ...
    config = Configuration('indriPy', parent_package, top_path)
    ...
    config.add_library('indri',
                       sources=[join('indri', 'xpdf', 'src', '*.cc'),
                                ...
                                join('indri', 'lemur', 'src', '*.c'), join('indri', 'lemur', 'src', '*.cpp'),
                                join('indri', 'src', '*.cpp')],
                       include_dirs=[join('indri', 'xpdf', 'include'), join('indri', 'antlr', 'include'),
                                     join('indri', 'lemur', 'include', 'lemur'),
                                     join('indri', 'include'), join('indri', 'lemur', 'include'),
                                     join('indri', 'xpdf', 'include'), join('indri', 'antlr', 'include')],
                       extra_compiler_args=extra_compile_args,
                       extra_link_args=['-stdlib=libstdc++', '-g', '-O3'],
                       )

    config.add_extension('query_env',
                         sources=['query_env.cpp'],
                         include_dirs=[numpy.get_include(), join('indri', 'include'), join('indri', 'lemur', 'include'),
                                       join('indri', 'xpdf', 'include'), join('indri', 'antlr', 'include')],
